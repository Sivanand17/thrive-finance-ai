...
const GoalTracker = ({ userId }: GoalTrackerProps) => {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'gadgets': return 'bg-blue-100 text-blue-800';
          ...
          <CardContent>
            <div className="space-y-6">
              {activeGoals.map((goal) => {
                ...
                const timeToGoal = goal.target_date ? getTimeToGoal(goal.target_date) : null;
                    ...
                    {editingId === goal.id && (
                      <div className="mt-4 flex gap-2">
                        <Input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              const additionalAmount = parseFloat((e.target as HTMLInputElement).value);
                              if (!isNaN(additionalAmount)) {
                                handleUpdateProgress(goal.id, goal.current_amount + additionalAmount);
                              }
                            }
                          }}
                        />
              ...
              {completedGoals.map((goal) => (
                <div key={goal.id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${getCategoryColor(goal.category)}`}>
                      {getCategoryIcon(goal.category)}
                    </div>
                    <div>
